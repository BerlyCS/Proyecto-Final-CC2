...
int main() {
    ...
    while (window.isOpen()) {
        ...
        // Detectar colisión entre el círculo y el rectángulo
        if (circle.getGlobalBounds().intersects(rectangle.getGlobalBounds())) {
            circle.setFillColor(sf::Color::Red); // Cambiar color del círculo a rojo
        } else {
            circle.setFillColor(sf::Color::Green); // Mantener color del círculo verde
        }
